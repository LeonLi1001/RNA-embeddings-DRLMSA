...
sequence_num = [3]
# dataset = load_dataset(f"dotan1111/MSA-nuc-{sequence_num[0]}-seq")


def main():
    config.device_name = "cuda:{}".format(sys.argv[1])
    config.device = torch.device(config.device_name)
    multi_train(sequence_num, 0, 25, truncate_file=True)
...
def multi_train(num_seq, start=0, end=-1, truncate_file=False):
    output_parameters()

    # iterate through all the datasets we want to train on
